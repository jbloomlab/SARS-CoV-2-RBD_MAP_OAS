# Mutational antigenic profiling of antibodies elicited by boosting of Wuhan-Hu-1 spike vaccination with BA.1 spike vaccination
This study was a collaboration with the [Victora lab](https://www.germinal.center/) at Rockefeller.

The Victora lab developed a system to model original antigenic sin (OAS) and to identify antibodies that are elicited as a result of boosting of a pre-existing antibody response, or are elicited _de novo_. They use a Cre-lox system to differentially tag antibodies that are boosting of a prior response (Stre-tagged) or antibodies that are elicited _de novo_ upon boosting (FLAG-tagged). Specifically, they prime mice twice with Wuhan-Hu-1 spike mRNA vaccine, then activate the Cre-lox system so that all antibodies that future B cell clones will produce FLAG-tag antibodies and then boost twice with BA.1 spike mRNA.
...
* This is rather aspirational, as there are some elements that are certainly hard-coded.
