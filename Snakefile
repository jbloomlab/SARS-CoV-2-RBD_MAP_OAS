...
# *antibody*, *target*, *secondary*, and *sort_bin*.
sample_vs_expect = (
    barcode_runs
    .assign(expect=lambda x: x[['antibody', 'target', 'secondary',
                                'sort_bin']]
...
rule make_summary:
    ...
    input:
        ...
        build_codon_variant_table=nb_markdown('build_codon_variant_table.ipynb'),
        mut_bind_expr=config['mut_bind_expr'],
        aggregate_variant_counts=nb_markdown('aggregate_variant_counts.ipynb'),
        variant_counts=config['variant_counts'].values(),
    ...
    run:
        ...
        with open(output.summary, 'w') as f:
            ...
            1. [Build codon variant table]({path(input.build_codon_variant_table)})
                from input files, and get the ACE2 binding and RBD expression
                [scores]({path(input.mut_bind_expr)}) for each library.

            4. Count variants and then
               [aggregate counts]({path(input.aggregate_variant_counts)})
               to create [variant counts file]({path(input.variant_counts[0])}).
...
rule escape_profiles:
    ...
    input:
        ...
        wildtype_sequence=config['wildtype_sequence'].values(),
        mut_bind_expr=config['mut_bind_expr'],
        strong_escape_sites=config['strong_escape_sites'],
    output:
...
rule counts_to_scores:
    ...
    input:
        config['variant_counts'].values(),
        config['wildtype_sequence'].values(),
        config['mut_bind_expr'],
    output:
        nb_markdown=nb_markdown('counts_to_scores.ipynb'),
        escape_scores=config['escape_scores'],
        escape_score_samples=config['escape_score_samples'].values(),
        escape_fracs=config['escape_fracs'],
    params:
...
rule aggregate_variant_counts:
    input:
        ...
        wt_seq=config['wildtype_sequence'].values(),
...
rule count_variants:
    ...
    input:
        ...
        wt_seq=config['wildtype_sequence'].values(),
        variant_table=config['codon_variant_tables'].values(),
    output:
    ...
    run:
        ...
        target = (barcode_runs_expandR1
                    .set_index('sample_lib')
                    .at[params.sample_lib, 'target']
                    )
        ...
        variant_table=config['codon_variant_tables'][target]
        ...
        variants=dms_variants.codonvarianttable.CodonVariantTable(
                    geneseq=geneseq,
                    barcode_variant_file=variant_table,
...
rule build_codon_variant_table:
    """Get input codon variant tables and combine into one codon variant table.
    And concatenate the ACE2 binding and expression scores for each library.
    """
    input:
        config['input_codon_variant_tables'].values(),
    output:
        config['codon_variant_table'],
        config['codon_variant_tables'].values(),
        file=config['mut_bind_expr'],
        nb_markdown=nb_markdown('build_codon_variant_table.ipynb')
    params:
        nb='build_codon_variant_table.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"
